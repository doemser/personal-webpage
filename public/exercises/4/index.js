...
if (stored) {
  ...
  note.innerHTML = "You are already subscribed";
  root.append(note);
} else {
...
<input type="text" name="a" value="tasdasd" readonly/>  <!-- Wird migeschickt, ist nicht editierbar -->
<input type="text" name="b" value="tasdasd" disabled/> <!-- Wird nicht mitgeschickt -->
<input type="hidden" name="csfrtoken" value="tasdasd"/> <!-- Tauch gar nicht erst auf-->
...
`;
  ...
  form.addEventListener("submit", event_ => {
    ...
    //?
    const formProps = Object.fromEntries(formData); // Suchst sich alles auf dem form raus und nicht nur den gezielten Wert den wir unter Umständen suchen
